...
/**
 * MySQL-flavored SqlDialect used for unparsing. Calcite's
 ...
 * syntax real MySQL rejects (the MySQL grammar has no ASYMMETRIC/SYMMETRIC
 * keyword after BETWEEN, unlike PostgreSQL). This dialect intercepts the
 * plain (ASYMMETRIC) form — negated or not; Calcite's {@code NOT_BETWEEN}
 * carries the same ASYMMETRIC flag — and renders the identical keyword
 * structure without the flag: plain {@code BETWEEN} and {@code NOT BETWEEN}
 * are both legal MySQL, so the semantics are unchanged either way.
 *
 * <p>The SYMMETRIC form (semantics MySQL cannot express) keeps Calcite's
 * default rendering, which the MySQL round-trip re-parse then rejects loudly
 * — it must never be silently downgraded to plain BETWEEN, because that
 * would change the meaning of the masked SQL.
 */
final class MysqlUnparseDialect extends MysqlSqlDialect {

  MysqlUnparseDialect() {
    super(MysqlSqlDialect.DEFAULT_CONTEXT);
  }
