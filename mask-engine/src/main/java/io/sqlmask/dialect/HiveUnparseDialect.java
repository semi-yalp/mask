...
/**
 * Hive-flavored SqlDialect used for unparsing. Calcite's
 ...
 * syntax real Hive rejects (Hive has no ASYMMETRIC/SYMMETRIC keyword after
 * BETWEEN, same gap as MySQL). This dialect intercepts the plain (ASYMMETRIC)
 * form — negated or not; Calcite's {@code NOT_BETWEEN} carries the same
 * ASYMMETRIC flag — and renders the identical keyword structure without the
 * flag: plain {@code BETWEEN} and {@code NOT BETWEEN} are both legal Hive,
 * so the semantics are unchanged either way.
 *
 * <p>The SYMMETRIC form (semantics Hive cannot express) keeps Calcite's
 * default rendering, which the Hive round-trip re-parse then rejects loudly
