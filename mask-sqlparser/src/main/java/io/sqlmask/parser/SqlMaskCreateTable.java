...
/**
 * Parse tree for {@code CREATE TABLE} statement，含 Babel 方言扩展
 * （{@code CREATE [SET|MULTISET] [VOLATILE] TABLE [IF NOT EXISTS] ... AS query}）。
 *
 * <p>与 calcite-babel 的 {@code org.apache.calcite.sql.babel.SqlBabelCreateTable}
 * 逐行对齐（task 5 差分验证），仅包名与枚举类型不同：枚举内嵌在本类，
 * 使本模块 main 无需依赖 test-scope 的 calcite-babel。unparse 输出与
 * Babel 完全一致，保证 BabelEquivalenceTest 的 toString 差分成立。
 */
public class SqlMaskCreateTable extends SqlCreateTable {

  /** 对齐 Babel TableCollectionType：UNSPECIFIED / SET / MULTISET。 */
  public enum TableCollectionType {
    UNSPECIFIED, SET, MULTISET
  }

  private static final SqlOperator OPERATOR =
      SqlBasicOperator.create("CREATE TABLE", SqlKind.CREATE_TABLE).withCallFactory(
          (operator, functionQualifier, pos, operands) ->
              new SqlMaskCreateTable(pos,
  ...
  /** Creates a SqlMaskCreateTable. */
  public SqlMaskCreateTable(SqlParserPos pos, boolean replace,
