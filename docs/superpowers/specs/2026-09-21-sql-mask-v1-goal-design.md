...
v2 目标与范围同日期定稿：见 `docs/superpowers/specs/2026-09-21-sql-mask-v2-goal-design.md`。v2 在 v1 基座上做服务化（SpringBoot 改写服务 + 五方言元数据导入 + 密码加密 + 解析缺陷修复 + 工程化 + Docker），**依旧只改写不查询**；策略仍为配置，不加服务化改造。v1 的模块与语义原样成为 v2 基座（§9 复用映射）。