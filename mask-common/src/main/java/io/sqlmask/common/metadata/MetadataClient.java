...
/**
 * Pull primitive over the metadata service data plane. Mirrors the policy
 * config source's transport discipline (X-Api-Key, mapped status codes,
 * fail-closed on unreachable). Caching/polling/propagation live with the
 * callers, not here. Used by mask-core (instance rewrite mode) and
 * mask-policy-server (metadata import).
 */
