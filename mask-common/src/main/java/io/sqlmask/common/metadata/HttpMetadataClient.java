...
/**
 * HTTP transport of {@link MetadataClient}: pulls over the metadata service
 * data plane. Mirrors the policy config source's transport discipline
 * (X-Api-Key, mapped status codes, fail-closed on unreachable).
 * Caching/polling/propagation live with the callers, not here.
 */
public final class HttpMetadataClient implements MetadataClient {
  ...
  public HttpMetadataClient(String baseUrl, String apiKey) {
