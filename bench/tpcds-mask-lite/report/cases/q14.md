...
|---|---|---|---|---|---|---|
| 仅脱敏 | ✓ | 0 | — | 5115 | 69.478 | same |
| 仅行过滤 | ✓ | 0 | — | 5650 | 117.300 | same |
| 脱敏+行过滤 | ✓ | 0 | — | 6002 | 123.383 | same |
...
item AS iws,
date_dim AS d3
WHERE ws_item_sk = iws.i_item_sk AND ws_sold_date_sk = d3.d_date_sk AND d3.d_year BETWEEN ASYMMETRIC 1999 AND 1999 + 2) AS sq1
...
FROM web_sales,
date_dim
WHERE ws_sold_date_sk = d_date_sk AND d_year BETWEEN ASYMMETRIC 1999 AND 1999 + 2) AS sq2) SELECT channel, i_brand_id, i_class_id, i_category_id, SUM(sales) AS sum_sales, SUM(number_sales) AS sum_number_sales
