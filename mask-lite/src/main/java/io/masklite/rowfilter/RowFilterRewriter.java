...
import io.masklite.dialect.DialectProfile;
 ...
 * PostgreSQL's search path is {@code CATALOG_SCHEMA}, so validation rejects
 * the reference anyway and there is nothing to inject. Name comparison
 * follows the profile's {@code caseSensitiveNameMatching}.
 *
...
public final class RowFilterRewriter {
  ...
  public RowFilterRewriter(Dialect dialect) {
  ...
  public Result apply(SqlNode parsed, MaskingConfig config, RowFilterRegistry registry) {
    if (registry.isEmpty()) {
      return new Result(parsed, 0);
    }
    Context context = new Context(config, registry);
  ...
  private static void rejectQualifiedColumnReferences(RowFilterRewriter rewriter, SqlNode parsed,
      Context context) {
    for (TableMetadata table : context.config.tables()) {
  ...
  private static final class Context {
    final MaskingConfig config;
    final RowFilterRegistry registry;
    int injections;

    Context(MaskingConfig config, RowFilterRegistry registry) {
      this.config = config;
  ...
  private SqlNode rewriteQuery(SqlNode node, Context context, Deque<Set<String>> cteScopes) {
    ...
    switch (node.getKind()) {
      ...
      default:
        // the pipeline only routes SELECT statements here; anything
        // unexpected passes through and faces the validator unchanged
        return node;
    }
  }
  ...
  private SqlNode resolveTableReference(SqlIdentifier reference, Context context,
    ...
    if (names.size() == 2 && resolveTwoPartNames) {
      ...
      // entry bind schema.table under the declared catalog, so the
  ...
  private boolean isVisibleCte(String name, Deque<Set<String>> cteScopes) {
    // comparison follows the same case rules as the validator
  ...
  /**
   * PostgreSQL identifier semantics: unquoted references were folded to
  ...
  private SqlNode injectIfFiltered(TableMetadata table, SqlIdentifier reference, Context context,
    ...
    // the registry is the single source of truth for "controlled"
