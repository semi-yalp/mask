...
import io.masklite.lineage.LineageStatus;
...
/**
 * The complete rewriting plan for one statement: one {@link OutputRewrite}
 * per output column, in output order. A wrapper is required only when at
 * least one output column matched a policy.
 */
public record RewritePlan(List<OutputRewrite> outputs) {
   ...
   */
  public static RewritePlan of(List<OutputLineage> lineage, ColumnMaskSelector selector) {
