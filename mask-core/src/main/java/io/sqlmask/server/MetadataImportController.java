...
public class MetadataImportController {
  ...
  public ImportResponse importMetadata(HttpServletRequest httpRequest,
    ...
    return audit.adminChange(httpRequest, "IMPORT", "TABLES", name, name,
        () -> Map.of("sourceInstance", request.metadataInstance(), "tableCount", tableCount.get()),
        () -> doImport(name, request, tableCount));
  }

  private ImportResponse doImport(String name, ImportRequest request, AtomicInteger tableCount) {
